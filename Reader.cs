...
namespace BlackPackageImageTool
{
    ...
    {
        public static void Pack(ushort value, byte[] buffer, int offset)
        {
   ...
   internal class Reader
        {
            ...
            public Reader (BinaryReader input, BlackPackageImageTool.Pt1MetaData info)
            {
                m_type = info.Type;
                m_input = new byte[info.PackedSize+8];
                input.BaseStream.Position = 0x20;
                if (info.PackedSize != input.Read (m_input, 0, info.PackedSize))
                    throw new InvalidCastException ("Unexpected end of file");
            ...
            void ReadNext ()
            {
                byte cl = (byte)(32 - ch);
                edx &= 0xFFFFFFFFu >> cl;
                edx += BitConverter.ToUInt32 (m_input, src) << ch;
                src += cl >> 3;
                ch += (byte)(cl & 0xf8);
            }
            ...
            void UnpackV2 ()
            {
                ...
                src += 3;
                dst += 3;
                edx = BitConverter.ToUInt32 (m_input, src);
                src += 3;
                ch = 0x18;
                uint _CF;
                uint ebx;
